...
def create_handler(args: adsk.core.CommandCreatedEventArgs):
    try:
        ...
        inputs.addBrowserCommandInput(
            'splineDataBtn', 'Spline Data', 'common/resources/browse.html', 28)

        # Spline rail chooser
        railRow = inputs.addButtonRowCommandInput(
            'railSplines', 'Rail Splines', True)
        command_inputs['leftSpline'] = railRow.listItems.add(
            'Left Spline', True, os.path.join(RESOURCES, 'rails', 'left'))
        command_inputs['centerSpline'] = railRow.listItems.add(
            'Center Spline', True, os.path.join(RESOURCES, 'rails', 'center'))
        command_inputs['rightSpline'] = railRow.listItems.add(
            'Right Spline', True, os.path.join(RESOURCES, 'rails', 'right'))
        ...
        # Start point
        start_point_input = inputs.addIntegerSpinnerCommandInput(
            'startPoint', 'Start Point', 1, 1000000, 1, 1)
        start_point_input.tooltip = 'Start point to draw (1 for beginning)'
        command_inputs['startPoint'] = start_point_input

        # End point
        end_point_input = inputs.addIntegerSpinnerCommandInput(
            'endPoint', 'End Point', 1, 1000000, 1, 1)
        end_point_input.tooltip = 'End point to draw (1 for beginning)'
        command_inputs['endPoint'] = end_point_input

        # Num points
        num_points_input = inputs.addIntegerSpinnerCommandInput(
            'numPoints', 'Points to Sketch', 1, 1000000, 1, 1)
