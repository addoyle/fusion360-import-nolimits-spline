...
def input_changed_handler(args: adsk.core.InputChangedEventArgs):
    start_point_input: adsk.core.IntegerSpinnerCommandInput = args.inputs.itemById(
        'startPoint')
    end_point_input: adsk.core.IntegerSpinnerCommandInput = args.inputs.itemById(
        'endPoint')
    num_points_input: adsk.core.IntegerSpinnerCommandInput = args.inputs.itemById(
        'numPoints')
    plane_input: adsk.core.SelectionCommandInput = args.inputs.itemById(
        'plane')
    ...
    if args.input.id == 'numPoints':
        num_points_input.value = min(
            num_points_input.value, spline.count - start_point_input.value + 1)
